...
class Encoder(nn.Module):
    def __init__(self, img_dim, in_channels, out_channels, head_num, mlp_dim, block_num, patch_dim):
        ...
        self.vit = ViT(self.vit_img_dim, out_channels * 16, out_channels * 16,
                       head_num, mlp_dim, block_num, patch_dim=1, classification=False)

        self.conv2 = nn.Conv2d(out_channels * 16, out_channels * 16, kernel_size=3, stride=1, padding=1)
        self.norm2 = nn.BatchNorm2d(out_channels * 16)
    ...
    def forward(self, x):
        ...
        x = self.vit(x)
        x = rearrange(x, "b (x y) c -> b c x y", x=self.img_dim[0]//self.patch_dim, y= self.img_dim[1]//self.patch_dim)
